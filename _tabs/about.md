---
# the default layout is 'page'
icon: fas fa-user-secret
order: 4
---

# Mouhamadou Lamine Ly


> *"Find what you love, what the world needs, what you're good at, and what sustains you — then pursue it relentlessly, one day at a time."*

---

## whoami

Cybersecurity student based in **Dakar, Senegal**, on a deliberate path toward offensive security. I operate under the philosophy of **Ikigai** — doing work that sits at the intersection of passion, purpose, and craft — and **Kaizen** — improving by 1% every single day, consistently.

![Ikigai](../assets/img/posts/ikigai.jpeg)

This blog is both a learning journal and a knowledge base. Every writeup, every technique breakdown, every note published here is a step in the process.

---

## ./skills

- **Focus areas** — Active Directory attacks, Web application security
- **Platforms** — HackTheBox `Hacker` · TryHackMe `Legend`
- **Certifications** — CNSP · CSCRB · CPPS

---

## ./certifications

| Badge | Certification                                   | Issuer           |
| ----- | ----------------------------------------------- | ---------------- |
| 🛡️   | CNSP — Certified Network Security Practitioner  | SecOps Group     |
| 🔐    | CSCRB — Certified Secure Code Review Beginners  | Red Team Leaders |
| 🔍    | CPPS — Certified Phishing Prevention Specialist | Hack & Fix       |

---

## ./philosophy

**Ikigai (生き甲斐)** — I pursue cybersecurity not just as a career, but as a calling. The intersection of curiosity, utility, and craft is where I operate best.

**Kaizen (改善)** — Mastery isn't a destination. Every machine rooted, every CVE studied, every post written is a small but deliberate improvement. The compound effect of consistent effort is what separates good from exceptional.

---

## ./contact

- **GitHub** — [github.com/0xnullbytes](https://github.com/0xnullbytes)
- **LinkedIn** — [linkedin.com/in/0xnullbytes](https://www.linkedin.com/in/0xnullbytes)

---

*From Dakar, with intent.*
